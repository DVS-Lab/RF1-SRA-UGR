...
%% Fit a line through each rejection.

x = [0.05,0.10,0.25,0.50]; % for fitting.

nonsocial_bins = [bin1(:,1), bin2(:,1), bin3(:,1), bin4(:,1)];
social_bins = [bin1(:,2), bin2(:,2), bin3(:,2), bin4(:,2)];

% make lines:

social_sensitivity = [];
nonsocial_sensitivity = [];

for kk = 1:length(social_bins)
    
    nonsocial_output = polyfit(x,social_bins(kk,:),1); % first output is slope. Second output is intercept.
    social_output = polyfit(x,nonsocial_bins(kk,:),1);
    
    social_sensitivity = [social_sensitivity; social_output(1)];
    nonsocial_sensitivity = [nonsocial_sensitivity; nonsocial_output(1)];
    
end

[R,P] = corrcoef(behavioral_data(:,2),social_sensitivity);

figure
scatter(behavioral_data(:,2),social_sensitivity)

ax = gca;
ax.FontSize = 12;
title ([''])
xlabel ('Age', 'FontSize', 16);
ylabel  ('Social Sensitivity (slope)', 'FontSize', 16);
...
saveas(gcf,'age_socialsens.svg')


[R,P] = corrcoef(behavioral_data(:,2),nonsocial_sensitivity);


figure
scatter(behavioral_data(:,2),nonsocial_sensitivity)

ax = gca;
ax.FontSize = 12;
title ([''])
xlabel ('Age', 'FontSize', 16);
ylabel  ('Nonsocial Sensitivity (slope)', 'FontSize', 16);
...
saveas(gcf,'age_nonsocialsens.svg')

[R,P] = corrcoef(behavioral_data(:,2),social_sensitivity-nonsocial_sensitivity);

figure
scatter(behavioral_data(:,2),social_sensitivity-nonsocial_sensitivity)

ax = gca;
ax.FontSize = 12;
title ([''])
xlabel ('Age', 'FontSize', 16);
ylabel  ('Social-Nonsocial Sensitivity (slope)', 'FontSize', 16);
...
saveas(gcf,'age_social-nonsocialsens.svg')
