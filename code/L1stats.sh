...
EV_ACCEPT=${EVDIR}_dec_accept.txt 
if [ -e $EV_ACCEPT ]; then
	SHAPE_ACCEPT=3
else
	SHAPE_ACCEPT=10
...
EV_REJECT=${EVDIR}_dec_reject.txt
if [ -e $EV_REJECT ]; then
	SHAPE_REJECT=3
else
	SHAPE_REJECT=10
fi
EV_ACCEPT_PMOD=${EVDIR}_dec_accept_pmod.txt 
if [ -e $EV_ACCEPT_PMOD ]; then
	SHAPE_ACCEPT_PMOD=3
else
	SHAPE_ACCEPT_PMOD=10
fi
EV_REJECT_PMOD=${EVDIR}_dec_reject_pmod.txt
if [ -e $EV_REJECT_PMOD ]; then
	SHAPE_REJECT_PMOD=3
else
	SHAPE_REJECT_PMOD=10
fi
EV_SOCIAL_ACCEPT=${EVDIR}_dec_social_accept.txt
if [ -e $EV_SOCIAL_ACCEPT ]; then
	SHAPE_SOCIAL_ACCEPT=3
else
	SHAPE_SOCIAL_ACCEPT=10
fi
EV_SOCIAL_REJECT=${EVDIR}_dec_social_reject.txt
if [ -e $EV_SOCIAL_REJECT ]; then
	SHAPE_SOCIAL_REJECT=3
else
	SHAPE_SOCIAL_REJECT=10
fi
EV_NONSOCIAL_ACCEPT=${EVDIR}_dec_nonsocial_accept.txt
if [ -e $EV_NONSOCIAL_ACCEPT ]; then
	SHAPE_NONSOCIAL_ACCEPT=3
else
	SHAPE_NONSOCIAL_ACCEPT=10
fi
EV_NONSOCIAL_REJECT=${EVDIR}_dec_nonsocial_reject.txt
if [ -e $EV_NONSOCIAL_REJECT ]; then
	SHAPE_NONSOCIAL_REJECT=3
else
	SHAPE_NONSOCIAL_REJECT=10
fi

EV_SOCIAL_ACCEPT_PMOD=${EVDIR}_dec_social_accept_pmod.txt
if [ -e $EV_SOCIAL_ACCEPT_PMOD ]; then
	SHAPE_SOCIAL_ACCEPT_PMOD=3
else
	SHAPE_SOCIAL_ACCEPT_PMOD=10
fi
EV_SOCIAL_REJECT_PMOD=${EVDIR}_dec_social_reject_pmod.txt
if [ -e $EV_SOCIAL_REJECT_PMOD ]; then
	SHAPE_SOCIAL_REJECT_PMOD=3
else
	SHAPE_SOCIAL_REJECT_PMOD=10
fi
EV_NONSOCIAL_ACCEPT_PMOD=${EVDIR}_dec_nonsocial_accept_pmod.txt
if [ -e $EV_NONSOCIAL_ACCEPT_PMOD ]; then
	SHAPE_NONSOCIAL_ACCEPT_PMOD=3
else
	SHAPE_NONSOCIAL_ACCEPT_PMOD=10
fi
EV_NONSOCIAL_REJECT_PMOD=${EVDIR}_dec_nonsocial_reject_pmod.txt
if [ -e $EV_NONSOCIAL_REJECT_PMOD ]; then
	SHAPE_NONSOCIAL_REJECT_PMOD=3
else
	SHAPE_NONSOCIAL_REJECT_PMOD=10
fi
...
if [ "$ppi" == "ecn" -o  "$ppi" == "dmn" ]; then
	...
	-e 's@SHAPE_ACCEPT@'$SHAPE_ACCEPT'@g' \
	-e 's@SHAPE_REJECT@'$SHAPE_REJECT'@g' \
	-e 's@SHAPE_ACCEPT_PMOD@'$SHAPE_ACCEPT_PMOD'@g' \
	-e 's@SHAPE_REJECT_PMOD@'$SHAPE_REJECT_PMOD'@g' \
	-e 's@SHAPE_SOCIAL_ACCEPT@'$SHAPE_SOCIAL_ACCEPT'@g' \
	-e 's@SHAPE_SOCIAL_REJECT@'$SHAPE_SOCIAL_REJECT'@g' \
	-e 's@SHAPE_NONSOCIAL_ACCEPT@'$SHAPE_NONSOCIAL_ACCEPT'@g' \
	-e 's@SHAPE_NONSOCIAL_REJECT@'$SHAPE_NONSOCIAL_REJECT'@g' \
	-e 's@SHAPE_SOCIAL_ACCEPT_PMOD@'$SHAPE_SOCIAL_ACCEPT_PMOD'@g' \
	-e 's@SHAPE_SOCIAL_REJECT_PMOD@'$SHAPE_SOCIAL_REJECT_PMOD'@g' \
	-e 's@SHAPE_NONSOCIAL_ACCEPT_PMOD@'$SHAPE_NONSOCIAL_ACCEPT_PMOD'@g' \
	-e 's@SHAPE_NONSOCIAL_REJECT_PMOD@'$SHAPE_NONSOCIAL_REJECT_PMOD'@g' \
